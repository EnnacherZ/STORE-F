    ...
    const load = async () => {
      try {
        const res = await connecter.get(
          `api/product/search/get?productType=${encodeURIComponent(productType ?? '')}&category=${encodeURIComponent(category ?? '')}&ref=${encodeURIComponent(ref ?? '')}&id=${encodeURIComponent(id ?? '')}`
        );
