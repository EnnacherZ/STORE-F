...
// Emoji per step key — the pure state logic itself lives in utils/orderSteps.ts
// (shared with the account order-history view), only the icon differs here.
const STEP_ICONS: Record<OrderStepResult["key"], string> = {
  placed: "🧾",
  payment: "💳", // overridden to 💵 for COD orders below
  processing: "📦",
  delivery: "🚚",
};

interface OrderStep extends OrderStepResult {
  icon: string;
}
...
function getOrderSteps(order: OrderData): OrderStep[] {
  ...
  return computeOrderSteps(order).map(step => ({
    ...step,
    icon: step.key === "payment" ? (isCod ? "💵" : "💳") : STEP_ICONS[step.key],
  }));
}
...
function getStoredAttempts(lsAttempts: string, lsReset: string): number {
  const raw   = localStorage.getItem(lsAttempts);
  const reset = localStorage.getItem(lsReset);

  if (reset && Date.now() - parseInt(reset, 10) > RESET_MS) {
    localStorage.setItem(lsAttempts, "0");
    localStorage.setItem(lsReset, Date.now().toString());
    return 0;
  }

  if (!reset) localStorage.setItem(lsReset, Date.now().toString());
  return raw ? JSON.parse(raw) : 0;
}

function bumpAttempts(current: number, lsAttempts: string, lsReset: string): number {
  const next = current + 1;
  localStorage.setItem(lsAttempts, JSON.stringify(next));
  if (current === 0) localStorage.setItem(lsReset, Date.now().toString());
  return next;
}
...
const OrderTracker: React.FC = () => {
  ...
  const [attempts,   setAttempts]   = useState(() =>
    getStoredAttempts(storageKeys.orderTrackingAttempts, storageKeys.orderTrackingLastReset)
  );
  ...
  const handleSearch = useCallback(
    async (overrideId?: string) => {
      ...
      const next = bumpAttempts(attempts, storageKeys.orderTrackingAttempts, storageKeys.orderTrackingLastReset);
      setAttempts(next);
    ...
    [query, isLoading, limitReached, attempts, storageKeys]
  );
