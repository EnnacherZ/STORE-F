...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const STORAGE_KEY = storageKeys.cartItems;
  const SUCCESS_KEY = storageKeys.successItems;
  ...
  const cartTotalAmount = useMemo(() => {
    const total = allItems.reduce(
      (sum, item) => sum + getLineTotal(item.price, item.promo, item.quantity),
      0
    );
